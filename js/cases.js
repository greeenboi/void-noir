const gameCases = [
	{
		...
		challenges: [
			{
				question:
					"Find all guests who entered restricted areas (security_level >= 3) around the time of the gunshot (between 14:20 and 14:35)",
				...
				validateFn: (result, expectedResult) => {
					...
					const resultData = result.rows
						.map((r) => `${r.name}-${r.zone_name}`)
						.sort();
					const expectedData = expectedResult.rows
						.map((r) => `${r.name}-${r.zone_name}`)
						.sort();
		...
		challenges: [
			{
				...
				validateFn: (result, expectedResult) => {
					...
					const resultData = result.rows
						.map((r) => `${r.name}-${r.zone_name}-${r.access_type}`)
						.sort();
					const expectedData = expectedResult.rows
						.map((r) => `${r.name}-${r.zone_name}-${r.access_type}`)
						.sort();
					return (
						resultData.length === expectedData.length &&
						resultData.every((data, i) => data === expectedData[i])
					);
				},
			},
			{
				question:
					"Analyze the evidence log and investigator notes to find any inconsistencies in the investigation",
				...
				validateFn: (result, expectedResult) => {
					...
					return (
						...
						resultIds.every((id, i) => id === expectedIds[i])
					);
				},
			},
		],
