...
class SQLDetectiveGame {
	...
	drawTransition(width, height) {
		const alpha =
			this.gameState.transitionDirection === "out"
				? this.gameState.transitionProgress
				: 1 - this.gameState.transitionProgress;
	...
	updateWalkingSound() {
		const isMoving =
			this.gameState.keyState.left || this.gameState.keyState.right;

		if (isMoving && !this.isWalking && this.gameState.scene === "hallway") {
			this.playSound("walking", true);
			this.isWalking = true;
		} else if (
			(!isMoving || this.gameState.scene !== "hallway") &&
			this.isWalking
		) {
			this.stopSound("walking");
			this.isWalking = false;
