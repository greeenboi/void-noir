...
class SQLDetectiveGame {
	constructor() {
		...
		this.gameState = {
			scene: "welcome", // welcome, cutscene, hallway, office, database, transition
			playerProgress: 0,
			caseSolved: false,
			cutsceneIndex: 0,
			cutsceneData: null,
			...
			portalActive: false, // Whether the exit portal is active
			transitionProgress: 0, // For transition animation
			transitionDirection: "in", // 'in' or 'out'
			nextCaseId: null, // Next case to load after transition
		};
		...
		this.assets = {
			...
			background: null,
			hallway: null,
			manSmall: null, // Add the new asset
	...
	loadAssets() {
		...
		// Load man-small for welcome screen
		...
		// No need to load computer anymore since we use a hitbox
	...
	async loadCase(caseId) {
		...
		// Ensure portal is not active when loading a new case
	...
	async checkChallengeSolution(query) {
		...
		const currentChallenge =
			this.currentCase.challenges[this.currentChallengeIndex];
		...
		if (currentChallenge.validateFn(userResult, expectedResult)) {
			...
			// Activate the portal in the hallway
	...
	showPortalMessage() {
		// Show a message about the portal
		this.showAlert(
			"A portal has appeared in the hallway! Return to proceed.",
			"info",
		);
	}
	...
	setupLittleJS() {
		...
		// Add keyboard listener for space key and arrow keys
	...
	startTransition(direction) {
		...
		// If transitioning out, prepare to load the next case when transition completes
		if (direction === "out" && this.gameState.nextCaseId) {
			// We'll load the next case when the transition is complete
		}
	}

	updateTransition() {
		// Update transition progress
		const speed = 0.02; // Speed of the transition
		...
		if (this.gameState.transitionProgress >= 1) {
			if (this.gameState.transitionDirection === "out") {
				...
				// Transition back to hallway scene
				this.gameState.scene = "hallway";
	...
	drawTransition(width, height) {
		// Create a white flash transition effect
		const alpha =
			this.gameState.transitionDirection === "out"
				? this.gameState.transitionProgress // Fade to white
				: 1 - this.gameState.transitionProgress; // Fade from white
	...
	drawWelcomeScreen(width, height) {
		...
		// Use man-small asset for welcome screen if available, otherwise fall back to detective
		if (this.assets.manSmall) {
			const imgWidth = Math.min(400, width * 0.3);
			const aspectRatio =
				this.assets.manSmall.height / this.assets.manSmall.width;
			const imgHeight = imgWidth * aspectRatio;
			const imgX = width / 2 - imgWidth / 2;
			const imgY = height / 2 - imgHeight / 2;

			// Apply noir filter effect
			this.ctx.drawImage(this.assets.manSmall, imgX, imgY, imgWidth, imgHeight);

			// Add a noir overlay for dramatic effect
			...
			// Fallback to detective silhouette if man-small isn't loaded
			const detectiveWidth = Math.min(400, width * 0.3);
			const aspectRatio =
				this.assets.detective.height / this.assets.detective.width;
	...
	drawCutscene(width, height) {
		const currentDialog =
			this.gameState.cutsceneData[this.gameState.cutsceneIndex];
	...
	drawHallwayScene(width, height) {
		...
		if (this.assets.hallway) {
			...
			const imgAspectRatio =
				this.assets.hallway.width / this.assets.hallway.height;
		...
		// Draw portal on the right if active - moved up by 250 pixels
		if (this.gameState.portalActive) {
			this.drawPortal(hallwayRight - 100, height - 550, 150, 280);
		}

		// Define invisible interaction zone in the hallway - moved up by 250 pixels
		const interactionZoneX = width / 2 - 100; // Center of the screen
		const interactionZoneY = height - 550; // Moved up by 250 pixels
		const interactionZoneWidth = 200;
		const interactionZoneHeight = 150;

		// Only draw the interaction zone in debug mode (for development purposes)
		const debugMode = false; // Set to true to see the hitbox
		if (debugMode) {
			this.ctx.fillStyle = "rgba(0, 255, 0, 0.3)";
			this.ctx.fillRect(
				interactionZoneX,
				interactionZoneY,
				interactionZoneWidth,
				interactionZoneHeight,
			);
		...
		if (this.assets.detective) {
			...
			// Determine detective's position for collision detection
			const detectiveX = this.gameState.detectivePos.x;
			const detectiveY = groundY - detectiveHeight;
			const detectiveCollisionWidth = 100; // Narrower than visual width for better feel
			...
			// Check for collision with interaction zone
			const detectiveRight = detectiveX + detectiveCollisionWidth;
			const detectiveLeft = detectiveX;

			// Calculate if detective is overlapping with interaction zone
			const isOverlapping =
			...
			if (isOverlapping) {
				// Show interaction prompt above detective
				this.ctx.fillStyle = "white";
				...
				this.ctx.fillStyle = `rgba(255, 255, 255, ${pulseIntensity})`;
				this.ctx.fillText(
					"Press SPACE to access terminal",
					detectiveX + detectiveCollisionWidth / 2,
					promptY,
				);

				// Add subtle glow effect around detective to indicate interaction is possible
		...
		// Update instructions based on whether portal is active
		if (this.gameState.portalActive) {
			this.ctx.fillText("Use arrow keys or A/D to move", width / 2, 50);
			this.ctx.fillText(
				"Look for the portal or terminal access points",
				width / 2,
				80,
			);
		} else {
			this.ctx.fillText("Use arrow keys or A/D to move", width / 2, 50);
			this.ctx.fillText(
				"Explore the hallway for database access points",
				width / 2,
				80,
			);
		}
		...
		// Check for collision with portal if it's active
		if (this.gameState.portalActive) {
			const portalX = hallwayRight - 100;
			const portalY = height - 550; // Updated position
			...
			const detectiveTop = groundY - 250; // Approximate top of detective
			const detectiveBottom = groundY; // Bottom of detective (at ground level)

			// Check for collision with both X and Y coordinates
			if (
				detectiveRight > portalX &&
				detectiveLeft < portalX + portalWidth &&
				detectiveBottom > portalY &&
				detectiveTop < portalY + portalHeight
			) {
				// Show interaction prompt - adjust Y position
				this.ctx.fillStyle = "white";
				this.ctx.font = "16px Arial";
				this.ctx.textAlign = "center";

				const promptY = portalY - 20; // Position prompt above portal
				const pulseIntensity = 0.7 + 0.3 * Math.sin(Date.now() / 300);
				this.ctx.fillStyle = `rgba(255, 255, 255, ${pulseIntensity})`;
				this.ctx.fillText(
	...
	drawOfficeScene(width, height) {
		...
		this.ctx.textAlign = "center";
		this.ctx.fillText(
			"Click on the computer to investigate case",
			width / 2,
			height / 2 + 150,
		);
		this.ctx.shadowBlur = 0;
	...
	drawPortal(x, y, width, height) {
		...
		// Create an animated, glowing door/portal
		...
		// Add some text above the portal
		this.ctx.font = "16px Arial";
		this.ctx.fillStyle = "white";
		this.ctx.textAlign = "center";
	...
	returnToOffice() {
		// Before returning to the hallway, create a visual cue that the detective
		// should now look for the next challenge's access point or portal
		this.gameState.scene = "hallway";
		this.sqlInterface.classList.add("hidden");

		if (this.gameState.portalActive) {
			this.showAlert(
				"Return to the portal to proceed to the next challenge",
				"info",
			);
		} else {
